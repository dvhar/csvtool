...
func runTests(doTest bool){
	...
	var tests = []Test {
		...
		Test{`select top 20 count(c1) + count(c2) / 2 count(c1) count(c2) sum(c1+c2) + avg(c1+c2) avg(c1+c2) c7 from`+f1+`group by month(c7) order by c7`,"expression of aggregates",true,selectSet},
